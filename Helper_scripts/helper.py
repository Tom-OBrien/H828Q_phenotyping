...
def filter_features_KEEP_BAD_WELLS(feat_df, meta_df, dates_to_drop=DATES_TO_DROP):
    """
...
def make_colormaps(gene_list, featlist, idx=[], candidate_gene=None, CONTROL_DICT={CONTROL_STRAIN:(0.6, 0.6, 0.6)}):
    ...
    strain_lut = {
        ...
        #             'add-1': ([0.93891607, 0.91319442, 0.80095805]), 
        #             'avr-14': ([0.94856697, 0.86769493, 0.76925477]), 
        #             'bbs-1': ([0.95462894, 0.81893441, 0.75835272]), 
        #             'bbs-2': ([0.9494023 , 0.75726888, 0.76389785]), 
        #             'cat-2': ([0.92363862, 0.69255273, 0.78297278]), 
        #             'cat-4': ([0.87965784, 0.63657336, 0.80461683]), 
        #             'dys-1': ([0.80599904, 0.57989676, 0.8254421 ]), 
        #             'figo-1': ([0.71044265, 0.53384447, 0.8334019 ]), 
        #             'glc-2': ([0.61320795, 0.50390334, 0.82336363]), 
        #             'glr-1': ([0.49922789, 0.48309788, 0.7884265 ]), 
        #             'glr-4': ([0.39116724, 0.47443003, 0.7274482 ]), 
        #             'gpb-2': ([0.30859287, 0.47416809, 0.6536652 ]), 
        #             'kcc-2': ([0.2383424 , 0.47789797, 0.55444091]), 
        #             'mpz-1': ([0.1965379 , 0.48066502, 0.44668002]), 
        #             'nca-2': ([0.18363848, 0.47807211, 0.35193386]), 
        #             'pink-1': ([0.19289808, 0.46558424, 0.25545096]), 
        #             'snf-11': ([0.22002937, 0.44042309, 0.17733935]), 
        #             'snn-1': ([0.25105097, 0.40688108, 0.12735562]), 
        #             'tmem-231': ([0.28377097, 0.35788714, 0.09406612]), 
        #             'tub-1': ([0.30447968, 0.30014738, 0.08238094]), 
        #             'unc-25': ([0.3066062 , 0.24517148, 0.08514552]), 
        #             'unc-43': ([0.28709658, 0.18385967, 0.09523206]), 
        #             'unc-49': ([0.24483442, 0.12770501, 0.10351877]), 
...
def select_drugs(candidate_drug, control, meta_df, feat_df=pd.DataFrame()):
    """
    ...
    drug_list = [d for d in meta_df.analysis.unique() if d != control]
    #gene_list = [g for g in gene_list if 'myo' not in g and 'unc-54' not in g]
    drug_list.sort()
    ...
    #     idx = [c for c,g in list(enumerate(gene_list)) if  g==candidate_gene]
    # else:
    if control not in candidate_drug:
        idx = [drug_list.index(item) for item in candidate_drug]
    else:
        idx=[]
        
    locs = list(meta_df.query('@candidate_drug in analysis').index)
    date_to_select = meta_df.loc[locs]['date_yyyymmdd'].unique()
    N2_locs = list(meta_df.query('@date_to_select in date_yyyymmdd and @control in analysis').index)
    locs.extend(N2_locs)

     #Only do analysis on the disease strains
    meta_df = meta_df.loc[locs,:]
    if feat_df.empty:
        return meta_df, idx, drug_list
    else:
        feat_df = feat_df.loc[locs,:]

        return feat_df, meta_df, idx, drug_list
    
def make_colourmaps_two_controls(drugs, CONTROL_STRAIN, TARGET_STRAIN, idx=[], candidate_drug=None, CONTROL_DICT={}):
    """
    ...
    cmap = list(np.flip((sns.color_palette('cubehelix',
                                           len(drugs)*2+6))[3:-4:2]))
    Target_cmap = ('salmon')
    Control_cmap = ('lightgrey')
    # Target_cmap = (0.6, 0.6, 0.6)
    # Control_cmap = (0.0, 0.0, 0.0)

    drug_lut = {}
    drug_lut[CONTROL_STRAIN] = CONTROL_DICT[CONTROL_STRAIN]
    drug_lut[TARGET_STRAIN] = CONTROL_DICT[TARGET_STRAIN]

    if candidate_drug is not None:
        for c,g in enumerate(candidate_drug):
            drug_lut[g] = cmap[idx[c]]

    else:
        cmap.append(Target_cmap)
        cmap.append(Control_cmap)
        drugs.append(CONTROL_STRAIN)
        drugs.append(TARGET_STRAIN)
        drug_zip = zip(drugs,cmap)
        drug_lut.update(dict(drug_zip))
        
    return drug_lut

def filter_features_no_annotations(feat_df, meta_df, dates_to_drop=DATES_TO_DROP):
    """
